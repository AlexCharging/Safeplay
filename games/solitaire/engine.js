...
function canPlaceFoundation(card,foundation){
  ...
  return (
    card.suit===top.suit &&
    getValue(card.value)===
    getValue(top.value)+1
  );
}

/* ---------------- SELECTION ---------------- */

export function selectTableau(
 state,
 col,
 index
){
...
export function moveToColumn(
 ...
 toCol
  ...
  if(state.selected.type==="tableau"){
    ...
    if(state.selected.col===toCol){
      state.selected=null;
      return false;
    }

    sourceCol=
      state.tableau[state.selected.col];
